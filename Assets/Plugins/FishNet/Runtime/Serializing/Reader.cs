...
namespace FishNet.Serializing
{
    ...
    public class Reader
    {
        ...
        public int ReadInt32(AutoPackType packType = AutoPackType.Packed)
        {
            int value = 0;
            if (packType == AutoPackType.Packed)
                value = (int)(long)ZigZagDecode(ReadPackedWhole());
            else
            {
                value = (int)ReadUInt32(packType);
            }
        ...
        public string ReadString()
        {
            ...
            {
                // Debug.LogWarning($"Read String Null, Size: {size}");
                return null;
            }
            else if (size == 0)
            {
                // Debug.LogWarning($"Read String Empty, Size: {size}");
                return string.Empty;
            }
            ...
            var str = ReaderStatics.GetString(data);
            // Debug.LogWarning($"Read String {str}, Size: {size}");
            return str;
        }
